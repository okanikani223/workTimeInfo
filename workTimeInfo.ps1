...
[String]::Join("`t", "日付", "曜日", "出勤時間", "退勤時間", "稼働時間(休憩時間:1hを除く)", "残業時間");
$workTimeInfo.workingDays | %{[String]::Join("`t", $_.date, $_.dayOfWeek, $_.boot, $_.shutdown, $_.workingTime, $_.overTime)};
[String]::Join("`t", "平日稼働日数", $workTimeInfo.subTotalWorkDays);
[String]::Join("`t", "平日稼働時間", $workTimeInfo.subTotalWorkTime.TotalHours);
[String]::Join("`t", "平日残業時間", $workTimeInfo.subTotalOverTime.TotalHours);
...
[String]::Join("`t", "休日残業時間", $workTimeInfo.subTotalHolidayOverTime.TotalHours);
[String]::Join("`t", "総稼働日数"  , $workTimeInfo.totalWorkDays);
[String]::Join("`t", "総稼働時間"  , $workTimeInfo.totalWorkTime.TotalHours);
[String]::Join("`t", "総残業時間"  , $workTimeInfo.totalOverTime.TotalHours);